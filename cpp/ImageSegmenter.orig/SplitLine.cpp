...
using namespace std;
 ...
 void ImageSegmenter::SplitLine(const Mat& img,vector < pair <unsigned int,unsigned int> > &WordBoundaries)
 {
	...
	*/
	...
	unsigned int white_row_threshold = static_cast<unsigned int>(static_cast<double>(img.rows) * m_white_col_fill_factor);
	...
	*/
	...
	for(unsigned int i = 1 ; i < col_sums.size() ;i++) {
		...
		if ( col_sums.at(i-1) >= white_row_threshold && col_sums.at(i) <= white_row_threshold ) 
			lineLeft = i;
		else if ( col_sums.at(i-1) <= white_row_threshold && col_sums.at(i) >=  white_row_threshold ) {
			lineRight = i;
	...
	wordEnd = WordBoundariesTemp.at(WordBoundariesTemp.size() - 1).second;
	pair<unsigned int, unsigned int> wordB;
	wordB.first = wordBegin;
	wordB.second = wordEnd;
	WordBoundaries.push_back(wordB);
